...
const TopBar = ({ route, cart }: any) => {
    ...
    return (
        ...
        >
            ...
            <Text style={{ textTransform: 'uppercase', fontSize: 18 }}>{route.params.category.name}</Text>
            <TouchableOpacity
                onPress={async () => {
                    // navigate('Auth
                }}
...
const CategoryScreen = ({ navigation: { navigate }, route }: any) => {
    ...
    React.useLayoutEffect(() => {
        setProductByCategory(
            product
                .filter((item: any) => item.category._id === route.params.category._id)
                .sort((a: any, b: any) => a.createdAt - b.createdAt)
        );
    }, [route]);

    const handleSelect = (value: any) => {
        if (!productByCategory?.length > 0) return;
        setLoading(true);
        switch (value) {
            case '0':
                setIsFilter('0');
                const sort = productByCategory.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

                setProductByCategory(sort);
                setTimeout(() => {
                    setLoading(false);
                }, 1000);
                break;
            case '1':
                setIsFilter('1');
                const sort1 = productByCategory.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));

                setProductByCategory(sort1);
                setTimeout(() => {
                    setLoading(false);
                }, 1000);
                break;
            case '2':
                setIsFilter('2');
                const sort2 = productByCategory.sort((a, b) => b.sold - a.sold);

                setProductByCategory(sort2);
                setTimeout(() => {
                    setLoading(false);
                }, 1000);
                break;
            case '3':
                setIsFilter('3');
                const sort3 = productByCategory.sort((a, b) => b.price - a.price);

                setProductByCategory(sort3);
                setTimeout(() => {
                    setLoading(false);
                }, 1000);
                break;
            case '4':
                setIsFilter('4');
                const sort4 = productByCategory.sort((a, b) => a.price - b.price);

                setProductByCategory(sort4);
                setTimeout(() => {
                    setLoading(false);
                }, 1000);
                break;
